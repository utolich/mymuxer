...
const RTP_RECV_BUFFER_BYTES: usize = 5 * 1024 * 1024;
const READ_CHUNK_SIZE: usize = 12 + 7 * 188;
...
pub async fn remote(
    ...
    helper.set_namespace(&format!("Input rtp #{}", worker_id));
    helper.log_play(&format!("Input rtp {} running", url));
    ...
    'send: loop {
        let response = tokio::select! {
            biased;
            _ = cancel.cancelled() => {
                helper.log_pause("Input rtp stopped");
                break 'send;
            },
                ...
                loop {
                    tokio::select! {
                        ...
                        res = timeout(input_timeout, res.recv_from(&mut buffer)) => {
                            match res {
                                Ok(Ok((n, _address))) => {
                                    let mut pkt = rtp_state.unpack(&buffer[..n])?;
...
async fn connect(url: &str, helper: &mut Helper) -> Result<UdpSocket> {
    ...
    match (u.host_str(), u.port()) {
        (Some(host), Some(port)) => {
            ...
            if let Err(e) = raw_socket.set_recv_buffer_size(RTP_RECV_BUFFER_BYTES) {
