...
pub async fn output(
    ...
    helper.set_namespace(&format!("Output udp #{}", output.id));
    helper.log_play(&format!("Output udp {} running", output.output_url));
            ...
            _ = timer.tick() => {
                ...
                if let Some(pkt) = packet_handler.flush_packet_for_cbr() {
                    match sock.send(&pkt).await {
                        ...
                        Err(e) => {
                            packet_handler.helper.log_warn(&format!("Output udp (cbr): send failed:{:?}", e));
                            break;
                        }
                    }
                }
    ...
    for _i in 1..=CONNECTIONS_REPEAT {
        match UdpSocket::bind("0.0.0.0:0").await {
            Ok(sock) => {
                ...
                if let Err(e) = sock_ref.set_send_buffer_size(UDP_SEND_BUFFER_BYTES) {
